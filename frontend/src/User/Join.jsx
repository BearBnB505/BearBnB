...
function Join() {
    ...
    //본인이 입력한 코드
    const [writeEmailCode, setWriteEmailCode] = useState('');
    //인증코드
    const [code, setCode] = useState();

    //현재 나이
    const [thisYearAge, setThisYearAge] = useState(0);
    ...
    // 인증번호 일치 테스트
    const onClickCodeCheck = (e) => {
        if(code===writeEmailCode) {
            Swal.fire({
                icon: 'success',
                title: '인증번호가 일치합니다',
    ...
    // 회원가입하기
    const onClickJoin = () => {
        if (insertFirstName === ''  || insertBirth === '' || insertTelNum === '' || insertLastName === '' ||
        password==='') {
            Swal.fire({
                icon: 'error',
                title: '빈 칸을 채워주세요',
        ...
        else if (emailMessage === '중복된 아이디입니다.') {
            Swal.fire({
                icon: 'error',
                title: '중복된 아이디는 가입할 수 없습니다',
            })
        }
        else if (code !== writeEmailCode) {
            Swal.fire({
                icon: 'error',
                title: '이메일 인증을 해주세요',
            })
        }
        // if(insertCountry===null || insertGender===null || insertBirth===null || insertTelNum===null || insertFinalName=null){
         else{
            axios({
                                    ...
                                    <div className={'row'}>
                                        ...
                                        <div className={'col-3 my-auto'}>
                                            <button className={'btn btn-primary'}
                                                    onClick={onClickEmailCheck}>중복체크
                                            </button>
                                        </div>
                                    </div>
